...
--
-- Staff grants are additive. Guided Change uses the existing guarded access
-- change RPC for selected-masjid replacement. The profile role is a cached
-- projection of capabilities effective on the current application date; it
-- is never used as the sole authorization boundary.
...
as $$
  with effective as (
    select coalesce(input_effective_date, public.current_effective_date()) as effective_date
  )
  select
    case
...
begin
  ...
  return private.recompute_profile_access(auth.uid(), public.current_effective_date());
end;
$$;
...
begin
  for profile_row in select profiles.id from public.profiles loop
    perform private.recompute_profile_access(profile_row.id, public.current_effective_date());
  end loop;
end;
$$;
...
as $$
  select coalesce((select projection.role = 'admin' and projection.active
    from private.raw_profile_access_projection(auth.uid(), public.current_effective_date()) as projection), false);
$$;
...
as $$
  select exists (
    ...
    from public.masjid_staff_memberships as memberships
    join public.masajid on masajid.id = memberships.masjid_id
    cross join lateral private.raw_profile_access_projection(auth.uid(), public.current_effective_date()) as projection
...
as $$
  select public.is_active_super_admin()
    or private.raw_is_admin_for_masjid(auth.uid(), input_masjid_id, public.current_effective_date());
$$;
...
begin
  ...
  if input_starts_on < public.current_effective_date() then
    raise exception using errcode = '22023', message = 'effective date cannot be historical.';
  end if;

  if input_preset = 'inactive' and input_starts_on <> public.current_effective_date() then
    raise exception using errcode = '22023', message = 'account deactivation must use the current application date.';
  end if;
  ...
  perform private.recompute_profile_access(input_target_profile_id, public.current_effective_date());

  select projection.role, projection.active
  into next_role, next_active
  from private.raw_profile_access_projection(input_target_profile_id, public.current_effective_date()) as projection;
