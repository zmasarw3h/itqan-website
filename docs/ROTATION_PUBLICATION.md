...
`apply_teacher_rotation_publication(...)` accepts that exact state and the desired assignment set, locks `rotation-publication:<cohort_id>:<week_start>`, locks relevant rows, recomputes state, and rejects a mismatch as stale. Group activity/order, settings, availability, staff dates, profile activity, assignments, cohort activity, and masjid activity invalidate a prepared plan.

Every assigned teacher needs a currently active profile, a teacher staff membership covering Saturday, an active masjid/cohort, and an exact `(teacher_id, cohort_id, week_start)` availability row with `available = true`. Missing availability means unavailable. Admin + Teacher capability is recognized by staff membership; no separate role exists.
...
`scripts/audit-rotation-publication-integrity.sql` is read-only and returns identifiers and reason codes only.
