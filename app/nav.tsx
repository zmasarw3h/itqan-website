...
import StudentShell from "@/app/student/student-shell";
...
export default async function AppNav({
  ...
  activeHref,
  studentPlacement,
  studentAssignmentPending = false
}: {
  role: Role;
  name: string;
  variant?: "default" | "rotation" | "teacher" | "workspace";
  activeHref?: string;
  studentPlacement?: string | null;
  studentAssignmentPending?: boolean;
}) {
  ...
  if (role === "student") {
    let resolvedPlacement = studentPlacement;
    let resolvedPending = studentAssignmentPending;
    if (studentPlacement === undefined) {
      const { supabase, profile } = await getCurrentProfile();
      const { weekStart } = currentWeeklyPlanContext();
      const scope = profile ? await loadStudentScopeForWeek(supabase, profile.id, weekStart) : null;
      resolvedPlacement = scope ? `${scope.cohortName} · ${scope.groupName}` : null;
      resolvedPending = !scope;
    }
    return <StudentShell identity={{ name, placement: resolvedPlacement ?? null, assignmentPending: resolvedPending }} />;
  }
