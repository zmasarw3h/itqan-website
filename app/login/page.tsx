...
export default async function LoginPage({
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-paper lg:grid lg:grid-cols-[minmax(0,45fr)_minmax(0,55fr)]">
      <section className="flex h-[22rem] min-w-0 flex-col bg-forest px-6 py-7 text-white sm:h-[23.5rem] sm:px-10 sm:py-9 lg:h-auto lg:min-h-dvh lg:px-[clamp(3.5rem,5vw,5rem)] lg:py-[clamp(4.75rem,9vh,6rem)]">
        <p className="text-2xl font-semibold uppercase tracking-[0.16em] text-gold-on-dark lg:text-[2.375rem]">
          ITQAN
        </p>
        ...
        <blockquote className="mx-auto flex w-full max-w-2xl flex-1 flex-col items-center justify-center pb-2 pt-4 text-center sm:pt-5 lg:pb-10 lg:pt-10">
          <p
            className={`${arabicFont.className} text-[clamp(2.5rem,11.25vw,2.75rem)] leading-[1.55] text-white sm:text-[3.5rem] lg:text-[clamp(3rem,5vw,4rem)] lg:leading-[1.6] xl:text-[clamp(4.5rem,5.15vw,5.35rem)]`}
            dir="rtl"
            lang="ar"
          >
          ...
          <div
            aria-hidden="true"
            className="my-4 flex w-44 items-center text-gold-on-dark sm:my-5 sm:w-48 lg:my-7 lg:w-full lg:max-w-[21rem]"
          >
          ...
          <cite className="mt-3 not-italic text-sm font-medium text-gold-on-dark sm:mt-5 sm:text-base lg:mt-8 lg:text-lg">
            Qur’an 73:4
          </cite>
        </blockquote>
      </section>
