...
import RotationStepScroll from "@/app/admin/rotation/rotation-step-scroll";
...
export default async function AdminRotationPage({ searchParams }: { searchParams: Promise<RotationSearchParams> }) {
  ...
  const prerequisites = readiness?.prerequisite_state;
  const unlocked = publishedPlanExists && !readiness
    ? { students: true, teachers: true, groups: true, review: true }
    : rotationWizardUnlockedSteps({
        studentsReady: prerequisites?.students.ready === true,
        teachersReady: prerequisites?.teachers.ready === true,
        groupsReady: prerequisites?.groups.ready === true && readiness?.source_stale === false,
        reviewReady: prerequisites?.review.ready === true && readiness?.source_stale === false
      });
  ...
  const status = params.status && statusAppliesToStep(params.status, step)
    ? ROTATION_STATUS_MESSAGES[params.status]
    : null;
  const absentCount = data.students.filter((student) => !student.available).length;
  const wizardTeachers = roster?.draft?.teachers ?? data.teachers.map((teacher) => ({
    teacher_id: teacher.id,
    teacher_name: teacher.name,
    teacher_email: teacher.email,
    available: teacher.available,
    last_published_week_start: teacher.last_published_week_start,
    last_published_halaqa_saturday: teacher.last_published_halaqa_saturday,
    last_published_group_name: teacher.last_published_group_name
  }));
  const availableTeacherCount = readiness?.available_teacher_count
    ?? wizardTeachers.filter((teacher) => teacher.available).length;
  ...
  return <>
    ...
    <main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-8 sm:py-8 lg:px-12">
      <header className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(26rem,34rem)] lg:items-end">
        <div><p className="text-sm font-semibold uppercase tracking-wide text-gold">Saturday halaqa operations</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Weekly rotation</h1><p className="mt-1 text-sm text-stone-600">{pageCopy}</p></div>
      ...
      <div className="mt-5">
        ...
        {data.context && step === "students" ? <section className="rounded-lg border border-stone-200 bg-white p-4 sm:p-6">
          ...
          <StudentAvailabilityForm cohortId={data.context.cohort.id} continueHref={pathFor("teachers")} initialConfirmed={publishedPlanExists || prerequisites?.students.confirmed === true} masjidId={data.context.masjid.id} saturdayLabel={formatHalaqaSaturday(data.selectedWeekStart)} students={data.students} weekStart={data.selectedWeekStart} />
        </section> : null}
        {data.context && step === "teachers" ? <section className="rounded-lg border border-stone-200 bg-white p-4 sm:p-6">
          ...
          <RotationAvailabilityProvider initialAvailableTeacherIds={wizardTeachers.filter((teacher) => teacher.available).map((teacher) => teacher.teacher_id)}><TeacherAvailabilityForm backHref={pathFor("students")} cohortId={data.context.cohort.id} continueHref={pathFor("groups")} initialConfirmed={publishedPlanExists || prerequisites?.teachers.confirmed === true} masjidId={data.context.masjid.id} teachers={wizardTeachers} weekStart={data.selectedWeekStart} /></RotationAvailabilityProvider>
        ...
        {data.context && roster && (step === "groups" || step === "review") ? <SessionRosterEditor actorNames={roster.actorNames} cohortId={data.context.cohort.id} initialDraft={roster.draft} initialHistory={roster.history} initialPublished={roster.published} key={step} legacyTransition={roster.legacyTransition} masjidId={data.context.masjid.id} paths={{ students: pathFor("students"), teachers: pathFor("teachers"), groups: pathFor("groups"), review: pathFor("review") }} step={step} weekStart={data.selectedWeekStart} /> : null}
      </div>
    </main>
  </>;
}

function statusAppliesToStep(status: string, step: RotationWizardStep) {
  if (status.startsWith("student-availability-")) return step === "students";
  if (["availability-saved", "save-error", "setup-incomplete"].includes(status)) return step === "teachers";
  return false;
}
