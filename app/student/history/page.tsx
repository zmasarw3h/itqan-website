...
export default async function StudentHistoryPage({
  ...
  const studentScope = studentContext.scope;

  if (!studentScope) {
    return (
      <StudentSetupIncomplete weekStart={selectedWeekStart} />
    );
  }
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                {day.checkin ? (
                  ...
                  <span
                    className={
                      day.isFuture
                        ? "rounded-md bg-stone-50 px-3 py-1.5 text-sm font-medium text-stone-600"
                        : "rounded-md bg-amber-50 px-3 py-1.5 text-sm font-medium text-amber-700"
                    }
                  >
                    {day.isFuture ? "Upcoming" : "Missing"}
                  </span>
                )}
              </div>

              {day.checkin?.note ? <p className="mt-4 break-words text-sm text-stone-700">Note: {day.checkin.note}</p> : null}
                    ...
                    {day.completedItems.length ? (
                      <ul className="mt-2 space-y-2">
                        {day.completedItems.map((item) => (
                          <li className="break-words rounded-md bg-green-50 px-3 py-2 text-sm text-green-900" key={item.id}>
                            {item.task_label}
                    ...
                    {day.missedItems.length ? (
                      <ul className="mt-2 space-y-2">
                        {day.missedItems.map((item) => (
                          <li className="break-words rounded-md bg-stone-50 px-3 py-2 text-sm text-stone-700" key={item.id}>
                            {item.task_label}
