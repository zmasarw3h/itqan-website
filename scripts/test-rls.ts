...
async function runAssertions(ids: SeedIds) {
  ...
  assert.deepEqual(
    ...
    [{ group_id: ids.groupA, week_start: ids.weekStart, roster_count: null }],
    "inactive hierarchy did not reduce a completed assignment to a label without roster access"
  );
